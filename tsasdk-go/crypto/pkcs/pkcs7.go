...
func (d *ParsedSignedData) verify(signer SignerInfo, opts x509.VerifyOptions) error {
	...
	// Verify signature
	algorithm := oid.ConvertToSignatureAlgorithm(signer.DigestAlgorithm.Algorithm, signer.SignatureAlgorithm.Algorithm)
	...
	if err := findAttribute(signer.SignedAttributes, oid.OIDAttributeContentType, &contentType); err != nil {
		return err
	}
	...
	if err := findAttribute(signer.SignedAttributes, oid.OIDAttributeMessageDigest, &expectedDigest); err != nil {
		return err
	}
	...
	if err := findAttribute(signer.SignedAttributes, oid.OIDAttributeSigningTime, &signingTime); err != nil {
