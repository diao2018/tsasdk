...
func ParseSignedData(data []byte) (*ParsedSignedData, error) {
	...
	if !oid.OIDSignedData.Equal(contentInfo.ContentType) {
...
func (d *ParsedSignedData) Verify(opts x509.VerifyOptions) ([]*x509.Certificate, error) {
	...
	for _, signer := range d.Signers {
		...
		thumbprint, err := digest.ComputeHash(crypto.SHA256, cert.Raw)
		if err != nil {
			return nil, err
		}
...
func (d *ParsedSignedData) verifySignature(signer SignerInfo, cert *x509.Certificate) error {
	...
	if err := signer.SignedAttributes.TryGet(oid.OIDAttributeContentType, &contentType); err != nil {
	...
	if err := signer.SignedAttributes.TryGet(oid.OIDAttributeMessageDigest, &expectedDigest); err != nil {
	...
	actualDigest, err := digest.ComputeHash(hash, d.Content)
	if err != nil {
	...
	if err := signer.SignedAttributes.TryGet(oid.OIDAttributeSigningTime, &signingTime); err != nil {
...
func getSignatureAlgorithmFromOID(digestAlg, sigAlg asn1.ObjectIdentifier) x509.SignatureAlgorithm {
	switch {
	case oid.OIDSignatureAlgorithmRSA.Equal(sigAlg):
		switch {
		case oid.OIDDigestAlgorithmSHA1.Equal(digestAlg):
			return x509.SHA1WithRSA
		case oid.OIDDigestAlgorithmSHA256.Equal(digestAlg):
			return x509.SHA256WithRSA
		case oid.OIDDigestAlgorithmSHA384.Equal(digestAlg):
			return x509.SHA384WithRSA
		case oid.OIDDigestAlgorithmSHA512.Equal(digestAlg):
			return x509.SHA512WithRSA
		}
	case oid.OIDSignatureAlgorithmRSASHA1.Equal(sigAlg):
		return x509.SHA1WithRSA
	case oid.OIDSignatureAlgorithmRSASHA256.Equal(sigAlg):
		return x509.SHA256WithRSA
	case oid.OIDSignatureAlgorithmECDSASHA384.Equal(sigAlg):
		return x509.SHA384WithRSA
	case oid.OIDSignatureAlgorithmECDSASHA512.Equal(sigAlg):
		return x509.SHA512WithRSA
	case oid.OIDSignatureAlgorithmECDSASHA1.Equal(sigAlg):
		return x509.ECDSAWithSHA1
	case oid.OIDSignatureAlgorithmECDSASHA256.Equal(sigAlg):
